...
function AdminFoglalasok() {
  ...
  const handleStatusChange = async (id: number, newStatus: 'accepted' | 'declined') => {
    try {
      ...
      toast.success(`Foglalás státusza frissítve: ${newStatus === 'accepted' ? 'Elfogadva' : 'Elutasítva'}`)
    } catch (err: unknown) {
      console.error(err)
  ...
  return (
    <div className="bookings-page-container">
      ...
      {bookings.length === 0 ? (
        <p className="no-bookings">Jelenleg nincs egyetlen foglalás sem a rendszerben.</p>
      ) : (
