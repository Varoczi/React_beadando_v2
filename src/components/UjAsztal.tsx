    ...
    <div className="modal-overlay">
      <div className="modal-content">
        ...
        <form onSubmit={handleSubmit}>
          ...
          <label>
            Típus:
            <select value={type} onChange={(e) => setType(e.target.value as TableType)}>
          ...
          <label>
            Kategória:
            <select value={category} onChange={(e) => setCategory(e.target.value as TableCategory)}>
          ...
          <label>
            Állapot ({status}):
            <input type="range" min="1" max="10" value={status} onChange={(e) => setStatus(Number(e.target.value))} />
          </label>
          ...
          <div className="modal-actions">
            <button type="button" onClick={onClose} className="cancel-btn">Mégse</button>
            <button type="submit" className="submit-btn">Tovább a lehelyezéshez</button>
          </div>
        </form>
      </div>
    </div>
